...
# create security group for the Jenkins Controller (formerly pipeline_agent_sg)
resource "aws_security_group" "jenkins_controller_sg" {
  name        = "${var.module_prefix}-jenkins-controller-sg"
  description = "Enable access for Jenkins Controller Web UI"
  vpc_id      = var.vpc_id
...
# create security group for the Jenkins Agent (worker node)
resource "aws_security_group" "jenkins_agent_sg" {
  name        = "${var.module_prefix}-jenkins-agent-sg"
  description = "Enable SSH access from Jenkins Controller for agent connection"
  vpc_id      = var.vpc_id
