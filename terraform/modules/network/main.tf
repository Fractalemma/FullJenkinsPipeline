...
#-- Jenkins Controller Subnet AZ A
resource "aws_subnet" "jenkins_sub_a" {
  ...
  tags = {
    Name = "${var.module_prefix}-jenkins-controller-sub-a"
  }
}
#-- Jenkins Agent Subnet AZ B
resource "aws_subnet" "jenkins_sub_b" {
  vpc_id                  = aws_vpc.vpc.id
  cidr_block              = var.jenkins_sub_b_cidr
  availability_zone       = data.aws_availability_zones.available_zones.names[1]
  map_public_ip_on_launch = true

  tags = {
    Name = "${var.module_prefix}-jenkins-agent-sub-b"
  }
}
...
# associate jenkins controller subnet jenkins_sub_a to "public route table"
resource "aws_route_table_association" "jenkins_sub_a_route_table_association" {
  subnet_id      = aws_subnet.jenkins_sub_a.id
  route_table_id = aws_route_table.public_route_table.id
}
# associate jenkins agent subnet jenkins_sub_b to "public route table"
resource "aws_route_table_association" "jenkins_sub_b_route_table_association" {
  subnet_id      = aws_subnet.jenkins_sub_b.id
  route_table_id = aws_route_table.public_route_table.id
}
